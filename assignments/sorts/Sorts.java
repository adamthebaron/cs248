public class Sorts {
	...
	public static void insertion(Comparable[] A, int gap, int offset) {
		for (int toinsert = gap + offset; toinsert < A.length; toinsert += gap) {
			Comparable item = A[toinsert];
	...
	private static void quickh(Comparable [] A, int start, int stop) {
		if (stop <= start)
		  return;
	...
	private static void mergeh(Comparable[] A, int start, int stop) {
    if (stop <= start)
		  return;
